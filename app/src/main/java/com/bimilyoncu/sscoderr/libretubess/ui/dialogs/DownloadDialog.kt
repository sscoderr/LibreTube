...
import com.bimilyoncu.sscoderr.libretubess.api.obj.PipedStream
...
import com.bimilyoncu.sscoderr.libretubess.databinding.DialogDownloadBinding
import com.bimilyoncu.sscoderr.libretubess.extensions.getWhileDigit
...
class DownloadDialog : DialogFragment() {
    ...
    private var videoStreams = listOf<PipedStream>()
    private var audioStreams = listOf<PipedStream>()
    private var subtitles = listOf<Subtitle>()
    
    // Default selections for video and audio modes
    private var defaultVideoSelection = 0
    private var defaultAudioSelection = 0
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        // Initially hide content and show loading indicator
        binding.contentLayout.visibility = View.GONE
        binding.loadingIndicator.visibility = View.VISIBLE

        // Setup radio button listeners
        binding.downloadTypeRadioGroup.setOnCheckedChangeListener { _, checkedId ->
            when (checkedId) {
                R.id.videoRadioButton -> {
                    updateForVideoMode()
                }
                R.id.audioRadioButton -> {
                    updateForAudioMode()
                }
            }
        }
        ...
        // Create dialog with a custom listener that will be set later
        val materialDialog = MaterialAlertDialogBuilder(requireContext())
            .setTitle(R.string.download)
            .setView(binding.root)
            .setPositiveButton(R.string.download) { _, _ -> 
                // This will be overridden but needed to create the button
            }
            .create()
        
        // Store reference and show the dialog
        dialog = materialDialog
        
        // Override the button click handler to prevent automatic dismiss
        ...
        // Start fetching streams after dialog is created
    ...
    private fun fetchAvailableSources() {
        lifecycleScope.launch {
            ...
            // Initialize options with the received response
    ...
    private fun initDownloadOptions(streams: Streams) {
        ...
        audioStreams = streams.audioStreams.filter {
            !it.url.isNullOrEmpty()
        }.sortedByDescending {
            it.quality.getWhileDigit()
        }
        ...
        // Find the best defaults for video and audio modes
        findDefaultSelections()
        
        // Video mode is selected by default (radio button is pre-checked in the layout)
        updateForVideoMode()
    ...
    private fun restorePreviousSelections(
        ...
        subtitles
            .indexOfFirst { it.code.orEmpty() == getSel(SUBTITLE_LANGUAGE) }
            .takeIf { it >= 0 }
            ?.let {
    ...
    /**
     * Find default selections for video and audio modes
     */
    private fun findDefaultSelections() {
        // For video mode: prefer 720p or closest resolution
        val target720p = 720
        defaultVideoSelection = videoStreams.indexOfFirst { 
            (it.quality.getWhileDigit() ?: 0) == target720p 
        }.takeIf { it >= 0 } ?: videoStreams.indexOfFirst { 
            (it.quality.getWhileDigit() ?: 0) <= target720p 
        }.takeIf { it >= 0 } ?: 0
        
        // For audio mode: prefer m4a format
        defaultAudioSelection = audioStreams.indexOfFirst { 
            it.format?.contains("m4a", ignoreCase = true) == true
        }.takeIf { it >= 0 } ?: 0 // Default to first item if m4a not found
    }
    
    /**
     * Update UI for video download mode
     */
    private fun updateForVideoMode() {
        if (::binding.isInitialized && videoStreams.isNotEmpty() && audioStreams.isNotEmpty()) {
            // Enable video spinner and set default selection
            binding.videoSpinner.isEnabled = true
            binding.videoSpinner.selectedItemPosition = defaultVideoSelection + 1
            
            // Set default audio selection for video mode
            binding.audioSpinner.selectedItemPosition = defaultAudioSelection + 1
        }
    }
    
    /**
     * Update UI for audio-only download mode
     */
    private fun updateForAudioMode() {
        if (::binding.isInitialized && audioStreams.isNotEmpty()) {
            // Disable video selection
            binding.videoSpinner.selectedItemPosition = 0 // No video
            binding.videoSpinner.isEnabled = false
            
            // Set default audio selection for audio mode
            binding.audioSpinner.selectedItemPosition = defaultAudioSelection + 1
        }
    }
    ...
    private fun dismissDialog() {
        // Handle dismissal in case of error
